...
def init_db():
    ...
    if session.query(Book).count() < 10:
        for i, data in enumerate(book_data):
            ...
            author_id = None
            # Пошук відповідного автора за іменем
            author = session.query(Author).filter_by(name=author_name).first()
            if author:
                author_id = author.id
            else:
                # Якщо автор не знайдений, додаємо нового
                new_author_data = next((author for author in author_data if author["name"] == author_name), None)
                if new_author_data:
            ...
            bookf = Book(title=data["title"], creator=data["creator"], description=data["description"], image_url=data["image_url"], author_id=author_id)
            session.add(bookf)
        session.commit()
    session.close()
